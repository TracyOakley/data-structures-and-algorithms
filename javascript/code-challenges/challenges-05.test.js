...
const addValues = (arr) => {
  return arr.reduce((accum,val)=> accum = val + accum,0);
};
...
const addPurchases = (arr) => {
  return arr.reduce((total,purchases) => total = total + purchases.purchasePrice, 0);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((total,item)=>total = total +1,0);
};
...
const returnNames = (arr) => {
  return(arr.reduce((accum,person) => {
    accum.push(person.name);
    return accum;
  },[]));
...
const reversedString = (str) => {
  return str.split('').reduce((rts, letter)=> {
    return letter + rts;
  },'');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((total,per)=> {
    if(per.children !== undefined){
      return total = total.concat(per.children);
    }else{
      return total;
    }
  }, []).length;
