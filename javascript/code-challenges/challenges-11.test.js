...
const count = (target, input) => {
  return input.reduce((total,item)=> {
    return total = total + item.reduce((innertotal,item)=> {
      return item === target ? innertotal + 1 : innertotal;
    },0);
  },0);
};
...
const totalSum = (input) => {
  return input.reduce((total,item)=> {
    return total = total + item.reduce((innertotal,item)=> {
      return innertotal = innertotal + item;
    },0);
  },0);
};
...
// && typeof(item)===Number
  ...
  let filteredArr =  input.map(arr => {
    return arr.filter(item => {
      return item%5===0 && typeof(item)==='number';
    });
  });
  return filteredArr.map(arrtwo => {
    return arrtwo.map( x => Math.pow(2,x));
  });
};
...
let findMaleAndFemale = (data) => {
  return data.filter(per => per.gender === 'male' || per.gender === 'female').reduce((names, per) => names +' and ' + per.name,'').replace(' and ','');
};
...
let findShortest = (data) => {
  return data.reduce((shortestPerson,per)=> {
   return parseInt(shortestPerson.height) > parseInt(per.height) ? per : shortestPerson;
  } , {name: 'Fake person',
  height: '500'}
  ).name;
};
